...
uses
  ...
  Forms,graphics,SHDocVw,Menus,Grids,
  inifiles,StdCtrls,
  OleCtrls,Controls, ExtCtrls;

type
  //-- results pile --
  TResultData = record
    px:string;
    volp,volm,sensvol,DVol:single;
    nb,qte,vol:integer;
  ...
  public
    ...
    //Evenements
    procedure OnCompleteLogin(Sender: TObject;const pDisp: IDispatch; var URL: OleVariant);
    procedure OnCompleteRGAFirst(Sender: TObject;const pDisp: IDispatch; var URL: OleVariant);
...
begin
  ...
  //Login
  WebB.OnDocumentComplete:=OnCompleteLogin;
  Web_Post('http://www.cotations.com/index.asp','User=gremy1&Password=ymer21AM',WebB);
end;
...
begin
  //Infos standard du titre capturé
  tbRGA.Text:=GetParsedValue(WebB.OleObject.Document.Body.innerHTML,'<TD vAlign=top align=left bgColor=#cccccc><FONT face=Arial,Helvetica size=2><B>','</B>',1);
  tbTitre.Text:=GetParsedValue(WebB.OleObject.Document.Body.innerHTML,'<TD vAlign=top noWrap align=left bgColor=#cccccc><FONT face=Arial,Helvetica size=2><B>','</B>',1);
  tbVarVeille.Text:=GetParsedValue(WebB.oleobject.document.body.innerHTML,'<TD vAlign=top align=middle bgColor=#cccccc><FONT face=Arial,Helvetica size=2>',' </FONT>',2);
  if pos('-',tbVarVeille.Text)<>0 then
    tbVarVeille.Font.color:=clRed
  else
    tbVarVeille.font.color:=clGreen;
  tbFirst.Text:=GetParsedValue(WebB.oleobject.document.body.innerHTML,'<TD vAlign=top align=middle bgColor=#cccccc><FONT face=Arial,Helvetica size=2>',' </FONT>',4);
  tbLast.text:=GetParsedValue(WebB.oleobject.document.body.innerHTML,'<TD vAlign=top align=middle bgColor=#cccccc><FONT face=Arial,Helvetica size=2>',' </FONT>',1);
  try
  ...
  tbLow.text:=GetParsedValue(WebB.oleobject.document.body.innerHTML,'<TD vAlign=top align=middle bgColor=#cccccc><FONT face=Arial,Helvetica size=2>',' </FONT>',6);
  tbHigh.text:=GetParsedValue(WebB.oleobject.document.body.innerHTML,'<TD vAlign=top align=middle bgColor=#cccccc><FONT face=Arial,Helvetica size=2>',' </FONT>',5);
  ...
  if FirstTime = false then begin
    ...
    if FResultsPile[0].qte>=0 then
      FResultsPile[0].volp:=FResultsPile[0].volp+FResultsPile[0].vol / 3.33
    else if FResultsPile[0].qte<0 then
      FResultsPile[0].volm:=FResultsPile[0].volm+FResultsPile[0].vol / 3.33;
    {Conditionnel nb}
    ...
    FResultsPile[0].DVol:=FResultsPile[0].sensvol-FResultsPile[1].sensvol;
    ...
    with FrmResults.sgResults do
      for i:=0 to MAX_RESULTS-1 do begin
        ...
        Cells[11,i+1]:=formatfloat('0.00',FResultsPile[i].DVol);
      end;
...
procedure TFrmMain.OnCompleteLogin(Sender: TObject;const pDisp: IDispatch; var URL: OleVariant);
var
  vFlags:OleVariant;
begin
  //Load RGA page in order to refresh it every sDelay seconds
  vFlags:=NavNoReadFromCache;
  WebB.OnDocumentComplete:=OnCompleteRGAFirst;
  WebB.Navigate('http://www.cotations.com/resultat_requete_cours_tempsreel.asp?VALEUR='+sRGA,vFlags);
end;

procedure TFrmMain.OnCompleteRGAFirst(Sender: TObject;const pDisp: IDispatch; var URL: OleVariant);
begin
